...
//TA: Raffaele Cetrul
//Date fo last Revision: 10/19/10
//Added the LCD GoTo and TermIO function
...
short TERMIO_PutChar(unsigned char letter){
  if(letter == CRLCD)
    return(LCD_Clear());
  else if(letter == LFLCD)
    blank();
  else if(letter == TABLCD){
    letter = ' ';
    LCD_OutChar(letter);
